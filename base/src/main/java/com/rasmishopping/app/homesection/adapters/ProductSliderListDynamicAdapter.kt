...
import com.rasmishopping.app.homesection.viewholders.ProductSliderDynamic
...
class ProductSliderListDynamicAdapter @Inject
constructor() : RecyclerView.Adapter<SliderItemTypeOne>() {
        ...
        if (jsonObject!!.getString("item_title").equals("1")){
            ...
            if(HomePageViewModel.isLightModeOn()){
                try{
                    ...
                    binding.name.setTextColor(Color.parseColor(NewBaseActivity.themeColor))
                }
            }
    ...
    override fun onBindViewHolder(item: SliderItemTypeOne, position: Int) {
        ...
        if (products?.get(position)?.node!!.images?.edges?.size!! > 0) {
            var url=products?.get(position)?.node!!.images?.edges?.get(0)?.node?.url
            if(url!=null){
                model.imageurl = products?.get(position)?.node!!.images?.edges?.get(0)?.node?.url
                Log.i("SaifDev_ImageQuery","URL"+products?.get(position)?.node!!.images?.edges?.get(0)?.node?.url)
                val height = products?.get(position)?.node!!.images?.edges?.get(0)?.node?.height
                val width = products?.get(position)?.node!!.images?.edges?.get(0)?.node?.width

...
import com.rasmishopping.app.utils.Urls.Data.MulipassSecret
...
class LoginViewModel(private val repository: Repository) : ViewModel() {
    ...
    val planResponse = MutableLiveData<ApiResponse>()
        ...
        when (reponse.status) {
            Status.SUCCESS -> {
                ...
                if (result.hasErrors) {
                    ...
                    Log.d(
                        TAG,
                        "consumeResponse: " + result.data!!.customerRecover.customerUserErrors.get(0).code
                    )
                    Log.d(
                        TAG,
                        "consumeResponse: " + result.data!!.customerRecover.customerUserErrors.get(0).field
                    )
                    Log.d(
                        TAG,
                        "consumeResponse: " + result.data!!.customerRecover.customerUserErrors.get(0).message
                    )
                    this.errormessage.setValue(errormessage.toString())
                } else {
                    Log.d(
                    ...
                    if (errors.size > 0) {
                        ...
                        Log.d(
                            TAG,
                            "consumeResponse: " + result.data!!.customerRecover.customerUserErrors.get(
                                0
                            ).code
                        )
                        Log.d(
    ...
    fun socialLogin(
        ...
        doRetrofitCall(
            repository.getUserLogin(mid, email,firstname,lastname),
            disposables,
            customResponse = object : CustomResponse {
                    ...
                    if (JSONObject(result.toString()).getBoolean("success")) {
                        if (JSONObject(result.toString()).getBoolean("is_present")) {
                            ...
                            if (present == true) {
                                ...
                                MagePrefs.saveSocialKey("Sociallogin")

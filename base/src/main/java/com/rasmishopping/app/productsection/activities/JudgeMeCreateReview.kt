...
import com.rasmishopping.app.databinding.ActivityJudgeMeCreateReviewBinding
...
class JudgeMeCreateReview : NewBaseActivity(), View.OnClickListener {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        binding = DataBindingUtil.inflate(
            layoutInflater,
            R.layout.activity_judge_me_create_review,
            group,
            true
        )
    ...
    private fun rateProduct() {
        if (TextUtils.isEmpty(binding?.nameEdt?.text?.toString()?.trim())) {
            ...
            var name_format_type=""
            if(binding?.reviewNameFormat?.selectedItem.toString().equals("John S."))
                name_format_type="last_initial"
            else if(binding?.reviewNameFormat?.selectedItem.toString().equals("J.S."))
                name_format_type="all_initials"
            else  if(binding?.reviewNameFormat?.selectedItem.toString().equals("Anonymous"))
                name_format_type="anonymous"
                ...
                Camera.REQUEST_TAKE_PHOTO -> if (resultCode === Activity.RESULT_OK) {
                    ...
                    if (selectedImage != null) {
                        ...
                        val requestFile: RequestBody = RequestBody.create(
                            getMimeType(Uri.fromFile(file))?.let { it.toMediaTypeOrNull() },
                            file
                        )
